    ...
    if(data) {
        if(data == 1) {
            ...
            if (check >= data && point_list.total1 != 0) {
                ...
                return res.status(200).json({
                    message: `You just received it ${point_list.total1}.00₹`,
                    status: true,
                    timeStamp: timeNow,
                });
        ...
        if(data == 2) {
            ...
            if (check >= get && point_list.total2 != 0) {
                ...
                return res.status(200).json({
                    message: `You just received it ${point_list.total2}.00₹`,
                    status: true,
                    timeStamp: timeNow,
                });
        ...
        if(data == 3) {
            ...
            if (check >= get && point_list.total3 != 0) {
                ...
                return res.status(200).json({
                    message: `You just received it ${point_list.total3}.00₹`,
                    status: true,
                    timeStamp: timeNow,
                });
        ...
        if(data == 4) {
            ...
            if (check >= get && point_list.total4 != 0) {
                ...
                return res.status(200).json({
                    message: `You just received it ${point_list.total4}.00₹`,
                    status: true,
                    timeStamp: timeNow,
                });
        ...
        if(data == 5) {
            ...
            if (check >= get && point_list.total5 != 0) {
                ...
                return res.status(200).json({
                    message: `You just received it ${point_list.total5}.00₹`,
                    status: true,
                    timeStamp: timeNow,
                });
        ...
        if(data == 6) {
            ...
            if (check >= get && point_list.total6 != 0) {
                ...
                return res.status(200).json({
                    message: `You just received it ${point_list.total6}.00₹`,
                    status: true,
                    timeStamp: timeNow,
                });
        ...
        if(data == 7) {
            ...
            if (check >= get && point_list.total7 != 0) {
                ...
                return res.status(200).json({
                    message: `You just received it ${point_list.total7}.00₹`,
                    status: true,
                    timeStamp: timeNow,
                });
...
const recharge = async(req, res) => {
    ...
    let transiction = req.body.transition;
    let type = req.body.type;
    let typeid = req.body.typeid;

    if (type != 'cancel') {
        if(!auth || !money || money < 999) {
            return res.status(200).json({
                message: 'Failed',
                status: false,
                timeStamp: timeNow,
    ...
    if (recharge.length == 0) {
        ...
        money = Number(money);
        let client_transaction_id = transiction;
        const formData = {
...
//                     today = ?,
//                     time = ?`;
//                     await connection.execute(sql, [id_time + '' + id_order, userInfo.phone, money, infoBank.stk, upi, infoBank.name_user, 0, checkTime, dates]);
...
const withdrawal3 = async(req, res) => {
    ...
    if (withdraw.length < 3) {
        if (userInfo.money - money >= 0) {
            let sql = `INSERT INTO withdraw 
                        (user_id, phone, money, stk, time) 
                        VALUES (?, ?, ?, ?, ?)`;
            await connection.execute(sql, [id_time + '' + id_order, userInfo.phone, money, upi, dates]);
            await connection.query('UPDATE users SET money = money - ? WHERE phone = ? ', [money, userInfo.phone]);
            return res.status(200).json({
                message: 'Withdraw money successfully',
                status: true,
