...
const addWinGo = async (game) => {
    try {
        ...
        if (!winGoNow || !winGoNow.length) {
            console.log('No data found for game:', join);
            return;
        }
...
const handlingWinGo1P = async (typeid) => {
    ...
    for (let i = 0; i < order.length; i++) {
        ...
        await connection.execute('UPDATE `minutes_1` SET `get_value` = ?, `status` = 1 WHERE `id` = ? ', [nhan_duoc, id]);
